...
export function ELDLogSheetFixed({
  date,
  driverName,
  vehicleNumber,
  carrierName,
    ...
    <div className="w-full max-w-6xl mx-auto">
      ...
      <div className="bg-white border-2 border-black p-4 mb-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <div className="border-b border-black min-h-[20px]">{totalMiles}</div>
          </div>
          <div>
            <label className="font-semibold">Truck or Tractor and Trailer Number:</label>
            <div className="border-b border-black min-h-[20px]">{vehicleNumber}</div>
          </div>
          <div>
            <label className="font-semibold">Name of Carrier:</label>
            <div className="border-b border-black min-h-[20px]">{carrierName}</div>
          </div>
          <div>
            <label className="font-semibold">Driver's Signature:</label>
            <div className="border-b border-black min-h-[20px]">{driverName}</div>
          </div>
          <div>
            <label className="font-semibold">Main Office Address:</label>
            <div className="border-b border-black min-h-[20px]">123 Trucking Way, Dallas, TX 75201</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="border-b border-black min-h-[20px]">
              {Object.values(statusTotals).reduce((a, b) => a + b, 0).toFixed(1)}h
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-black">
        ...
        <div className="border-b-2 border-black">
          <div className="flex">
            <div className="w-1/4 border-r border-black p-2 text-center font-semibold">
              Off Duty
            </div>
            <div className="w-1/4 border-r border-black p-2 text-center font-semibold">
              Sleeper Berth
        ...
        <div className="relative">
            ...
            {hours.map((hour) => (
              ...
              >
                <div className="text-xs text-gray-500 mt-1 ml-1">
                  {hour === 0 ? "Midnight" : hour < 12 ? `${hour} AM` : hour === 12 ? "Noon" : `${hour - 12} PM`}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex">
            {/* Off Duty Column */}
            <div className="w-1/4 border-r border-black relative min-h-[200px]">
              {dutyStatuses
                .filter((status) => status.status === "off_duty")
                .map((status, index) => (
                  <div
                    key={index}
                    className={`absolute top-0 h-full ${getStatusColor(status.status)} border`}
                    style={{
                      left: `${getHourPosition(status.start_time)}%`,
                      width: `${getHourPosition(status.end_time) - getHourPosition(status.start_time)}%`,
                    }}
                  >
                    <div className="p-1 text-xs">
                      <div className="font-semibold">{formatTime(status.start_time)}</div>
      ...
      <div className="bg-white border-2 border-black p-4 mt-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <span className="ml-2">{statusTotals.off_duty?.toFixed(1) || "0.0"}h</span>
          </div>
          <div>
            <label className="font-semibold">Total Sleeper Berth:</label>
            <span className="ml-2">{statusTotals.sleeper_berth?.toFixed(1) || "0.0"}h</span>
          </div>
          <div>
            <label className="font-semibold">Total Driving:</label>
            <span className="ml-2">{statusTotals.driving?.toFixed(1) || "0.0"}h</span>
          </div>
          <div>
            <label className="font-semibold">Total On Duty:</label>
            <span className="ml-2">{statusTotals.on_duty?.toFixed(1) || "0.0"}h</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-black p-4 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className={`ml-2 ${(statusTotals.driving || 0) > 11 ? "text-red-600" : "text-green-600"}`}>
              {(statusTotals.driving || 0).toFixed(1)}h / 11h
              {(statusTotals.driving || 0) > 11 ? " ❌ VIOLATION" : " ✅ Compliant"}
            </span>
          </div>
          <div>
